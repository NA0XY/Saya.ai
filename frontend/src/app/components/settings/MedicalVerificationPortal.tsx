    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-sketch font-bold text-[#1A1A1A] relative -top-1.5">Medical Verification</h3>
        ...
        <div className="flex items-center gap-2 bg-transparent sketch-box border-[#1A1A1A] px-3 py-1.5" style={{ transform: 'rotate(0.5deg)' }}>
          <div className="w-2 h-2 sketch-dot bg-[#1A1A1A] animate-pulse" />
          <span className="text-[10px] font-sketch font-bold uppercase tracking-widest text-[#1A1A1A]">HITL Required</span>
        </div>
      </div>

      <div className="space-y-4">
        {medicines.map((med, i) => (
          <div key={med.id} className="bg-white sketch-box p-5 text-[#1A1A1A] shadow-sm" style={{ transform: `rotate(${i % 2 === 0 ? -0.2 : 0.2}deg)` }}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-white text-[#1A1A1A] sketch-avatar flex items-center justify-center font-sketch font-bold text-base">{i+1}</div>
                <span className={`px-3 py-0.5 sketch-box font-sketch text-[10px] font-bold uppercase tracking-wider ${med.verified ? "text-green-600 border-green-600" : "text-[#1A1A1A] border-[#1A1A1A]"}`}>
            ...
            <div className="grid gap-4">
              <div className="relative">
                <div className="relative mb-2">
                  <label className="inline-block text-[10px] font-sketch font-bold uppercase tracking-widest text-[#1A1A1A]/70">Drug Name</label>
                  <div className="absolute -bottom-0.5 left-0 w-12 sketch-underline border-[#1A1A1A]"></div>
                </div>
                <div className="sketch-box text-[#1A1A1A]">
                  <input type="text" value={query[med.id]!==undefined && focusId===med.id ? query[med.id] : med.drugName}
                    ...
                    className="w-full px-3 py-2 bg-transparent focus:outline-none font-sketch text-lg font-bold text-[#1A1A1A] placeholder-[#1A1A1A]/40" />
                </div>
                {focusId===med.id && (
                  <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white sketch-box text-[#1A1A1A] shadow-lg max-h-40 overflow-y-auto">
                    {filtered(med.id).map(d => (
                      <button key={d} onMouseDown={()=>{update(med.id,"drugName",d);setFocusId(null);setQuery({...query,[med.id]:""});}} className="w-full text-left px-3 py-2 hover:bg-[#1A1A1A]/5 font-sketch text-base font-bold transition-colors">{d}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="relative mb-2">
                    <label className="inline-block text-[10px] font-sketch font-bold uppercase tracking-widest text-[#1A1A1A]/70">Dosage</label>
                  </div>
                  <div className="sketch-box text-[#1A1A1A]">
                ...
                <div>
                  <div className="relative mb-2">
                    <label className="inline-block text-[10px] font-sketch font-bold uppercase tracking-widest text-[#1A1A1A]/70">Frequency</label>
                  </div>
                  <div className="sketch-box text-[#1A1A1A]">
      ...
      <div className="mt-8 p-6 bg-white sketch-box text-[#1A1A1A] flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm" style={{ transform: 'rotate(-0.1deg)' }}>
        ...
        <button onClick={confirmAll} disabled={isSaving||allOk} className="w-full sm:w-auto px-8 py-3 bg-[#E85D2A] text-white border-black sketch-box font-sketch font-bold uppercase tracking-widest text-sm hover:rotate-[0.5deg] hover:scale-105 transition-all shadow-md disabled:opacity-40 disabled:cursor-not-allowed">
